RSpec.describe AuthorityBrowse do
  context "#load_terms_db" do
    it "loads terms and counts into the names table of the terms_db" do
      skip "Terms loading solr stubs not ready"
      params =
        {
          :q => "*:*",
          "terms.limit" => 1_000,
      ...
      expect(AuthorityBrowse::DB.db[:names_from_biblio].count).to eq(10)
      first = AuthorityBrowse::DB.db[:names_from_biblio].first
